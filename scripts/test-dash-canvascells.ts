...
import { cellKey } from '../dash/src/cellformula.ts'
...
{
  const d = fresh()
  const p: Patch = { op: 'setCanvasCells', sheet: 'c1', cells: { [cellKey(0, 0)]: { v: 'A' } } }
  const { inverse } = applyPatch(d, p)
  ok(sheet(d).cells['0,0'].v === 'A', 'a cell is written at cellKey(row, col) — the key cellformula.ts already uses')
...
{
  const d = fresh()
  applyPatch(d, { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': { v: 1 }, '5,9': { v: 2 } } })
  ok(keys(d).length === 2, 'two distant cells cost two entries')
  ok(!('1,1' in sheet(d).cells),
...
}
{
  ...
  for (let i = 0; i < 50; i++) many[cellKey(i, 0)] = { v: i }
...
{
  const d = fresh()
  applyPatch(d, { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': { v: 'x' }, '1,0': { v: 'y' } } })
  const { inverse } = applyPatch(d, { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': null } })
  ok(keys(d).join(',') === '1,0', 'null REMOVES the cell rather than storing an empty object')
  applyPatch(d, inverse as Patch)
  ok(sheet(d).cells['0,0'].v === 'x', 'and undo puts the value back, not a blank')
}
{
  // `null` and not `undefined`: this is what survives JSON, so it is what a
  // collaborator receives. Both must delete.
  const d = fresh()
  applyPatch(d, { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': { v: 'x' } } })
  applyPatch(d, { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': undefined as unknown as null } })
...
// ------------------------------------------------------------ refusals
{
  ...
  let why = ''
  try { applyPatch(d, { op: 'setCanvasCells', sheet: 't1', cells: { '0,0': { v: 1 } } }) } catch (e) { why = String(e) }
  ok(/no spreadsheet sheet/.test(why),
    'writing spreadsheet cells to a DATASET sheet is refused BY KIND, not silently coerced')
  ok(!(d.sheets[1] as { cells?: Record<string, unknown> }).cells?.['0,0'],
    'and the dataset sheet is untouched by the attempt')
  let why2 = ''
  try { applyPatch(d, { op: 'setCanvasCells', sheet: 'nope', cells: { '0,0': { v: 1 } } }) } catch (e) { why2 = String(e) }
...
{
  const d = fresh()
  const p: Patch = { op: 'setCanvasCells', sheet: 'c1', cells: { '0,0': { v: 1 } } }
