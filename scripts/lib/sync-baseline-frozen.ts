...
//
// ===========================================================================
//  FROZEN BASELINE — the bento-sync engine as shipped. DO NOT EDIT. EVER.
// ===========================================================================
//
//  This is a verbatim copy of slides/src/sync/crdt.ts, taken at the commit
//  that froze it, and it is the BASELINE half of scripts/test-sync-equiv.ts.
//  Its whole purpose is to be the thing that does not move while the engine
//  next door is parameterized to serve bento/spaces as well as bento/slides.
//
//  WHY A COPY, AND NOT THE FILE ITSELF. The rig used to import both halves
//  from slides/src/sync/crdt.ts, so it compared the engine to itself and
//  passed trivially. Worse, it compared MODULE IDENTITY to decide whether the
//  run was meaningful — so the moment the engine moves to the kernel and that
//  path becomes a re-export facade, the rig would announce "LIVE RUN" and
//  "PASS" while still comparing one engine with itself. Measured: a candidate
//  containing nothing but `export * from '…/crdt.ts'` produced exactly that.
//  A frozen copy cannot do this, because it stops tracking the engine the
//  instant the engine changes.
//
//  IF THE RIG GOES RED, THE ANSWER IS NEVER TO EDIT THIS FILE. A red run
//  means the candidate engine mints different bytes than the code that wrote
//  every bento/slides file already on someone's disk. Editing the baseline to
//  agree would delete the only evidence of that. scripts/test-sync-equiv.ts
//  asserts this file's sha256 against a recorded constant for that reason.
//
...
//  byte — and the freezing commit proves it, by running the rig with this
//  file as baseline against the untouched engine as candidate and requiring
//  a byte-identical PASS.
//
// SPDX-License-Identifier: MIT
// Copyright (c) 2026 The Bento authors
...
// Structural stand-ins for the slides model types (see the note above).
// Type-only: erased before this file runs.
type BentoDoc = Record<string, unknown> & { slides: Slide[] }
type Slide = Record<string, unknown> & { id: string; elements: SlideElement[] }
type SlideElement = Record<string, unknown> & { id: string }
