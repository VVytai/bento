...
import { renderPage, paintCode } from './render'
...
const AUTOFORMAT: Array<[RegExp, string, (b: Block, m: RegExpExecArray) => void]> = [
  [/^# $/, 'h1', () => {}],
  [/^## $/, 'h2', () => {}],
  [/^### $/, 'h3', () => {}],
  [/^- $/, 'bullet', () => {}],
  ...
  //
  // It is now completed by a SPACE, where a bare ``` used to convert on the
  // third backtick. That is a deliberate change to an existing trigger, for two
  // reasons: every other rule in this table is space-completed (`# `, `- `,
  // `> `, `--- `), and while ``` converts instantly there is no keystroke left
  // in which to type the language — which is the muscle memory everyone brings
  // from GitHub, Slack and every other markdown box.
  //
  // The tag is stored VERBATIM, not normalised. `lang` is document data, so a
  // language this build cannot highlight still round-trips, still exports as
  // ```rust, and lights up by itself the day the lexer learns it.
  [/^```(\w*) $/, 'code', (b, m) => { if (m[1]) b.lang = m[1] }],
  [/^--- $/, 'divider', () => {}],
]
