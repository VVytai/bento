  ...
  function renderSheets(): void {
    ...
    for (const sheet of store.doc.sheets) {
      ...
      if (isTable(sheet)) {
        ...
        // A canvas sheet is in the format from commit one and has no renderer
        // yet. Showing it greyed is honest; hiding it would make a sheet that
        // is really there look deleted.
        item.classList.add('dp-sheet-off')
        meta.textContent = t('canvas sheet — not editable in this build')
      }
